...
export async function POST(request: NextRequest) {
  try {
    ...
    const items = await fetchPayrollItems(
      ...
      periodStart,
      periodEnd,
      `basic_salary, employees!inner(employee_id_no, ssnit_number, nia_number, last_name, first_name, middle_name, tier2_applicable)`,
    )

    const rows = items
      .filter((item: any) => item.employees?.tier2_applicable !== false)
      .map((item: any) => {
        ...
        return {
          ...
          tier2_contribution: Number((Number(item.basic_salary || 0) * 0.05).toFixed(2)),
          code: 'N',
        }
      })
    ...
    await cacheReport(client, company.id, payPeriod, 'ssnit_tier2', reportPayload, companyId)
