...
export default function EmployeesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Import Data Dialog - Already defined below */}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                <div>
                  <h4 className="font-medium text-blue-900">Employee Import Template</h4>
                  <p className="text-sm text-blue-700">
                    Download and fill the template with employee data in CSV format
                  </p>
                </div>
                <Button
                  variant="outline"
              ...
              {previewData.length > 0 && (
                <div className="p-4 bg-green-50 rounded-lg">
                  <h4 className="font-medium text-green-900 mb-2">
                    Preview ({previewData.length} of {previewData.length} rows)
                  </h4>
                      ...
                      <thead>
                        <tr className="border-b border-green-200">
                          {Object.keys(previewData[0] || {})
                            ...
                            .map((key) => (
                              <th key={key} className="text-left p-2 font-medium text-green-900">
                                {key}
                              </th>
                            ))}
                        </tr>
                        ...
                        {previewData.slice(0, 5).map((row, index) => (
                          <tr key={index} className="border-b border-green-100">
                            {Object.entries(row)
                              .filter(([key]) => key !== "_rowIndex")
                              .map(([key, value], i) => (
                                <td key={i} className="p-2 text-green-800">
                                  {String(value)}
                                </td>
                              ))}
                          </tr>
