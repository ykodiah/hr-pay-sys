...
export default function AttendancePage() {
  ...
  const [provisioningSteps, setProvisioningSteps] = useState([
    {
      id: "register",
      title: "Register device with vendor",
      description: "Confirm the device is activated in the vendor portal and reachable on your network.",
      completed: true,
    },
    {
      id: "keys",
      title: "Store credentials securely",
      description: "Upload API keys, SFTP credentials, or certificates into the credential vault.",
      completed: false,
    },
    {
      id: "mapping",
      title: "Map device to subsidiaries",
      description: "Link the device to subsidiaries and locations for policy routing.",
      completed: false,
    },
    {
      id: "policies",
      title: "Attach automation policies",
      description: "Assign device-specific policies for sync SLAs and tamper monitoring.",
      completed: false,
    },
  ])
  const [deviceCredentials, setDeviceCredentials] = useState([
    {
      id: "visionx-sdk",
      name: "VisionX SDK key",
      type: "SDK",
      owner: "IT Integrations",
      maskedValue: "vk-live-****-42A9",
      lastRotated: new Date(Date.now() - 6 * 24 * 3600 * 1000).toISOString(),
    },
    {
      id: "zkteco-webhook",
      name: "ZKTeco webhook secret",
      type: "Webhook",
      owner: "Security Ops",
      maskedValue: "whsec_****_9F2B",
      lastRotated: new Date(Date.now() - 13 * 24 * 3600 * 1000).toISOString(),
    },
  ])
  const [healthCheckState, setHealthCheckState] = useState({
    frequency: "Every 30 minutes",
    lastRun: new Date(Date.now() - 15 * 60 * 1000).toISOString(),
    status: "passing" as "passing" | "warning" | "critical",
    issuesDetected: 0,
  })
  const [importTemplates] = useState([
    {
      id: "csv-basic",
      name: "CSV attendance snapshot",
      description: "Standard CSV layout for offline imports. Includes employee ID, timestamps, and geo coordinates.",
      size: "12 KB",
    },
    {
      id: "sftp-batch",
      name: "SFTP batch (JSON)",
      description: "JSON template for nightly SFTP drops. Supports metadata for device health and tamper flags.",
      size: "8 KB",
    },
  ])
  ...
  const handleToggleProvisioningStep = (stepId: string) => {
    let toggledStep: ProvisioningStep | null = null
    setProvisioningSteps((previous) =>
      previous.map((step) => {
        if (step.id === stepId) {
          const next = { ...step, completed: !step.completed }
          toggledStep = next
          return next
        }
        return step
    ...
    if (toggledStep) {
      toast({
        title: toggledStep.completed ? "Step completed" : "Step reopened",
        description: toggledStep.completed
          ? `${toggledStep.title} marked as complete.`
          : `${toggledStep.title} reopened for review.`,
      })
    }
  }

  const handleRotateCredential = (credentialId: string) => {
    let credentialName = ""
    setDeviceCredentials((previous) =>
      previous.map((credential) => {
        if (credential.id === credentialId) {
          credentialName = credential.name
          return { ...credential, lastRotated: new Date().toISOString() }
        }
        return credential
      }),
    )
    ...
    toast({
      ...
      description: credentialName
        ? `${credentialName} rotation logged. Update downstream integrations if required.`
        : "Credential rotation logged.",
    })
  }

  const handleRunHealthCheck = () => {
    const outcomes: Array<HealthCheckState["status"]> = ["passing", "warning", "critical"]
    const nextStatus = outcomes[Math.floor(Math.random() * outcomes.length)]
    const issuesDetected = nextStatus === "passing" ? 0 : nextStatus === "warning" ? 1 : Math.floor(Math.random() * 3) + 1

    setHealthCheckState({
      frequency: healthCheckState.frequency,
    ...
    toast({
      ...
      description:
        nextStatus === "passing"
          ? "All devices responded within SLA."
          : issuesDetected === 1
            ? "One device requires attention."
            : `${issuesDetected} devices require attention.`,
      variant: nextStatus === "critical" ? "destructive" : nextStatus === "warning" ? "default" : "default",
    })
  }
  ...
  const handleDownloadTemplate = (templateId: string) => {
    ...
    if (!template) {
      toast({
        ...
        description: "Could not find the import template selected.",
        variant: "destructive",
      })
      return
    }
    ...
    if (typeof window === "undefined") {
      toast({
        ...
        description: "Template downloads are only available in the browser.",
        variant: "destructive",
      })
      return
    }
    ...
    const sampleContent = [
      ...
      "",
      "employee_id,device_id,clock_in,clock_out,location",
      "EMP-001,DEV-001,2025-02-07T08:00:00Z,2025-02-07T17:00:00Z,Accra HQ",
      "EMP-004,DEV-003,2025-02-07T19:55:00Z,2025-02-08T06:05:00Z,Tema Plant",
    ].join("\n")
    ...
    anchor.href = url
    anchor.download = template.name.replace(/\s+/g, "-").toLowerCase() + ".txt"
    anchor.click()
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1.4fr)]">
              <Card className="border border-emerald-100">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base text-emerald-700">
                    <Sparkles className="h-4 w-4" /> Guided provisioning checklist
                  </CardTitle>
                  <CardDescription>
                    Follow the recommended rollout steps to keep devices compliant and observable.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {provisioningSteps.map((step) => (
                    <div
                      key={step.id}
                      className={cn(
                        "rounded border px-3 py-2 text-sm shadow-sm transition",
                        step.completed ? "border-emerald-200 bg-emerald-50/70" : "border-slate-200 bg-white",
                      )}
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge
                          className={step.completed ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-600"}
                        >
                      ...
                      <Button
                        ...
                        onClick={() => handleToggleProvisioningStep(step.id)}
                      >
                        {step.completed ? "Mark as pending" : "Complete step"}
                      </Button>
                    </div>
                  ))}
                </CardContent>
              ...
              <Card className="border border-slate-200">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base text-slate-900">
                    <ShieldCheck className="h-4 w-4 text-emerald-500" /> Credential vault
                  </CardTitle>
                  <CardDescription>
                    Securely store webhook secrets, SDK tokens, and SFTP credentials for connected devices.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-slate-600">
                  {deviceCredentials.map((credential) => (
                    <div key={credential.id} className="rounded border border-slate-200 bg-white p-3 shadow-sm">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-medium text-slate-900">{credential.name}</p>
                          <p className="text-[11px] uppercase text-slate-500">{credential.owner}</p>
                        </div>
                        <Badge variant="secondary" className="bg-slate-100 text-slate-700">
                          {credential.type}
                        </Badge>
                      </div>
                      <div className="mt-2 rounded bg-slate-50 px-2 py-1 font-mono text-xs text-slate-600">
                        {credential.maskedValue}
                      </div>
                      <p className="mt-1 text-[11px] text-slate-500">
                        Last rotated {new Date(credential.lastRotated).toLocaleString()}
                      </p>
                      <Button
                        size="sm"
                        variant="outline"
                  ...
                  {!deviceCredentials.length && (
                    <p className="text-xs text-slate-500">Add credentials to begin tracking rotation cadences.</p>
                  )}
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <Card className="border border-sky-100">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base text-sky-700">
                    <Radar className="h-4 w-4" /> Automated health checks
                  </CardTitle>
                  <CardDescription>
                    Scheduled pings ensure devices respond within SLA and surface anomalies proactively.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-slate-600">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="font-medium text-slate-900">{healthCheckState.frequency}</p>
                    </div>
                    <Badge
                      className={cn(
                        "px-3",
                        healthCheckState.status === "passing"
                          ? "bg-emerald-100 text-emerald-700"
                          : healthCheckState.status === "warning"
                            ? "bg-amber-100 text-amber-700"
                            : "bg-rose-100 text-rose-700",
                      )}
                    >
                  ...
                  <div className="grid gap-1 text-xs text-slate-500">
                    <span>Last run {new Date(healthCheckState.lastRun).toLocaleString()}</span>
                    <span>
                      Issues detected:{" "}
                      <span
                        className={
                          healthCheckState.issuesDetected
                            ? healthCheckState.status === "critical"
                              ? "text-rose-600"
                              : "text-amber-600"
                            : "text-emerald-600"
                        }
                  ...
                  <Button size="sm" onClick={handleRunHealthCheck}>
                    Run health check now
                  </Button>
                  <div>
                    <p className="text-xs uppercase text-slate-500">Recent breaches</p>
                    <div className="mt-2 space-y-2">
                      {slaBreaches.slice(0, 3).map((device) => (
                        <div key={`health-${device.id}`} className="rounded border border-slate-200 bg-white px-3 py-2 text-xs">
                          <p className="font-semibold text-slate-900">{device.name}</p>
                          <p className="text-slate-500">
                            {device.status.toUpperCase()} • {device.minutesSinceSync >= 999 ? ">999" : device.minutesSinceSync}m since sync
                          </p>
                        </div>
                      ))}
                      {!slaBreaches.length && (
                        <p className="text-xs text-slate-500">No SLA breaches detected in the last run.</p>
                      )}
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border border-slate-200">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base text-slate-900">
                    <Download className="h-4 w-4 text-emerald-500" /> Fallback import templates
                  </CardTitle>
                  <CardDescription>
                    Use these templates when terminals go offline or when onboarding historic punches.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-slate-600">
                  {importTemplates.map((template) => (
                    <div key={template.id} className="rounded border border-slate-200 bg-white p-3 shadow-sm">
                  ...
                  {!importTemplates.length && (
                    <p className="text-xs text-slate-500">Add import templates to support offline ingestion.</p>
                  )}
                </CardContent>
              </Card>
            </div>
